"""
Joorvis High — l'assistant complet.
...
.env :
    ...
    CITY=Paris          # optionnel, ville par défaut
    BRIEF=7:30          # optionnel, brief du matin ; "off" pour couper

Ce que la version mid n'a pas :
  · photos lues par un modèle de vision, documents (txt, md, csv, pdf)
  · notes, Wikipédia, recherche approfondie en un seul appel
  · brief du matin rédigé par le modèle, avec repli hors ligne
  · /export de toutes tes données, /purge avec confirmation
  · reprise automatique quand le modèle bafouille
"""
...
# modèle multimodal ; s'il disparaît du catalogue Groq, seule cette ligne bouge
VISION_MODEL = os.getenv("VISION_MODEL",
                         "meta-llama/llama-4-scout-17b-16e-instruct")

MAX_TURNS = 12         # paires user/assistant conservées
TOKEN_BUDGET = 6000    # au-delà, l'historique est élagué par le début
TTL = 90 * 60          # inactivité avant oubli du fil
MAX_STEPS = 8          # garde-fou de la boucle d'outils
MAX_FAITS = 120        # mémoire longue durée : plafond de fiches
MAX_DOC = 12000        # caractères lus dans un document envoyé
RETRIES = 3            # tentatives sur un appel modèle qui échoue
...
CREATE TABLE IF NOT EXISTS notes (
    id TEXT PRIMARY KEY, chat_id INTEGER, texte TEXT, cree TEXT);
...
EXPECTED = {
    ...
    "facts": {"chat_id": "INTEGER", "cle": "TEXT", "valeur": "TEXT",
              "maj": "TEXT"},
    "notes": {"id": "TEXT", "chat_id": "INTEGER", "texte": "TEXT",
              "cree": "TEXT"},
...
def save_note(chat_id: int, texte: str) -> str:
    nid = uuid.uuid4().hex[:6]
    with db() as con:
        con.execute("INSERT INTO notes VALUES (?,?,?,?)",
                    (nid, chat_id, texte.strip()[:4000],
                     now().isoformat(timespec="seconds")))
    return nid


def find_notes(chat_id: int, motif: str = "", limite: int = 10) -> list[dict]:
    q = "SELECT * FROM notes WHERE chat_id = ?"
    p: list = [chat_id]
    if motif.strip():
        q += " AND texte LIKE ?"
        p.append(f"%{motif.strip()}%")
    q += " ORDER BY cree DESC LIMIT ?"
    p.append(limite)
    with db() as con:
        return [dict(r) for r in con.execute(q, p)]


def drop_note(chat_id: int, nid: str) -> dict | None:
    with db() as con:
        row = con.execute("SELECT * FROM notes WHERE id = ? AND chat_id = ?",
                          (nid, chat_id)).fetchone()
        if row is None:
            return None
...
async def lire_page(url: str, limite: int = 4000) -> dict:
    ...
    h = re.sub(r"(?is)<(script|style|nav|footer|header|aside|form).*?</\1>", " ",
               r.text)
    titre = re.search(r"(?is)<title[^>]*>(.*?)</title>", h)
    txt = re.sub(r"&[a-z]+;|&#\d+;", " ", re.sub(r"(?s)<[^>]+>", " ", h))
    txt = re.sub(r"\s+", " ", txt).strip()
    return {"titre": titre.group(1).strip()[:120] if titre else "",
            "texte": txt[:limite], "tronque": len(txt) > limite, "url": url}


@tool
async def web(
    action: Annotated[str, "search, news, read, ou deep (cherche puis lit "
                           "les 2 meilleures pages en une seule fois)"],
    ...
    Utilise deep quand la réponse demande de vraiment lire les sources."""
    if action == "read":
        r = await lire_page(q)
        return r if "error" in r else {"_": WEB_AVERT, **r}
    ...
    if action == "deep":
        pages = await asyncio.gather(
            *(lire_page(r["u"], 2500) for r in res[:2] if r["u"]))
        return {"_": WEB_AVERT, "r": res,
...
@tool
async def wikipedia(
    sujet: Annotated[str, "Titre de l'article"],
    langue: Annotated[str, "Code langue : fr ou en"] = "fr",
) -> dict:
    """Résumé d'un article Wikipédia. Fiable pour les faits encyclopédiques."""
    lg = langue.lower()[:2] if langue else "fr"
    try:
        r = await http.get(f"https://{lg}.wikipedia.org/api/rest_v1/"
                           f"page/summary/{quote(sujet)}")
        if r.status_code == 404:
            return {"error": f"aucun article pour {sujet}"}
        d = r.json()
    except Exception as e:
        return {"error": f"wikipédia indisponible : {e}"}
    return {"titre": d.get("title"), "resume": d.get("extract"),
...
@tool
async def notes(
    action: Annotated[str, "add, search ou drop"],
    texte: Annotated[str, "add : la note ; search : les mots à retrouver"] = "",
    id: Annotated[str, "drop : identifiant obtenu via search"] = "",
    ctx=None, chat_id=None,
) -> dict:
    """Carnet de notes libres : idées, comptes rendus, extraits, listes.
    Plus long qu'une tâche, moins structuré qu'un fait mémorisé."""
    if action == "add":
        if not texte.strip():
            return {"error": "texte requis"}
        nid = save_note(chat_id, texte)
        ctx.chat_data["undo"] = ("note", nid, texte.strip()[:60])
        return {"ok": True, "confirmation": "📝 noté"}
    if action == "search":
        ns = find_notes(chat_id, texte)
        return {"notes": [{"id": n["id"], "date": n["cree"][:10],
                           "texte": n["texte"][:500]} for n in ns]} \
            if ns else {"notes": [], "note": "aucune note"}
    if action == "drop":
        n = drop_note(chat_id, id)
        return {"confirmation": "🗑 note supprimée"} if n \
            else {"error": f"aucune note {id}"}
    return {"error": "action inconnue, attendu add, search ou drop"}
...
SYSTEM = (
    ...
    "ou l'autre est une tâche ; un texte à garder est une note.\n"
    "Mémorise avec memoire ce qu'il révèle durablement sur lui, sans le dire.\n"
    "Enchaîne plusieurs outils quand la question le demande, sans t'arrêter "
    "pour commenter entre chaque.\n"
...
LABELS = {"web": "🔎 web", "crypto": "₿ cours", "reminders": "⏰ rappel",
          ...
          "wikipedia": "📚 wikipédia", "notes": "📝 notes"}


async def appel_modele(messages: list, outils: bool = True):
    """Appel Groq avec reprise : un 429 ou une coupure réseau ne perd pas le tour."""
    dernier = None
    for essai in range(RETRIES):
        try:
            kw = {"tools": SCHEMAS} if outils else {}
            return await asyncio.to_thread(
                groq.chat.completions.create, model=CHAT_MODEL,
                messages=messages, reasoning_effort="low", temperature=0.6, **kw)
        except Exception as e:
            dernier = e
            if essai == RETRIES - 1:
                break
            attente = 1.5 * (essai + 1)
            log.warning("groq (essai %d/%d) : %s — reprise dans %.1f s",
                        essai + 1, RETRIES, e, attente)
            await asyncio.sleep(attente)
    raise dernier


async def think(text_in: str, update: Update,
                context: ContextTypes.DEFAULT_TYPE, memo: str = "") -> None:
    chat_id = update.effective_chat.id
    hist = history(context)
    umsg = {"role": "user", "content": text_in}
    hist.append(umsg)
    ...
    try:
        for step in range(MAX_STEPS):
            try:
                resp = await appel_modele([system_msg(chat_id), *hist])
            except Exception as e:
                log.error("groq : %s", e)
                answer = ("Le modèle ne répond pas. Tes rappels et tes tâches "
        ...
        compact(hist)
        # le contenu d'un document ne reste pas dans l'historique : il a servi,
        # il ne sera pas repayé à chaque tour suivant
        if memo:
            umsg["content"] = memo
...
async def on_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Lit une photo avec le modèle de vision, puis garde le résumé dans le fil."""
    await context.bot.send_chat_action(update.effective_chat.id, ChatAction.TYPING)
    f = await update.message.photo[-1].get_file()
    b64 = base64.b64encode(bytes(await f.download_as_bytearray())).decode()
    question = (update.message.caption or "").strip() or "Décris cette image."
    status = await update.message.reply_text("👁  lecture …")
    try:
        vis = await asyncio.to_thread(
            groq.chat.completions.create, model=VISION_MODEL, temperature=0.3,
            messages=[{"role": "user", "content": [
                {"type": "text", "text":
                    f"{question}\nRéponds en français, texte brut, sans "
                    f"markdown. Si l'image contient du texte, transcris-le."},
                {"type": "image_url",
                 "image_url": {"url": f"data:image/jpeg;base64,{b64}"}}]}])
    except Exception as e:
        log.error("vision : %s", e)
        await edit(status, "Je n'arrive pas à lire cette image.")
        return
    desc = (vis.choices[0].message.content or "").strip() or "Image illisible."
    if vis.usage:
        stat("llm", vis.usage.prompt_tokens, vis.usage.completion_tokens)
    hist = history(context)
    hist.append({"role": "user", "content": f"[photo] {question}"})
    hist.append({"role": "assistant", "content": desc})
    await deliver(status, update, desc)


def texte_document(nom: str, brut: bytes) -> str | None:
    """Extrait le texte d'un document simple. None si le format est inconnu."""
    bas = nom.lower()
    if bas.endswith(".pdf"):
        try:
            import io
            ...
            return None
        try:
            pages = PdfReader(io.BytesIO(brut)).pages
            return "\n".join(p.extract_text() or "" for p in pages[:40])
        except Exception:
            return None
    if bas.endswith((".txt", ".md", ".csv", ".json", ".log", ".py", ".yml",
                     ".yaml", ".ics", ".srt")):
...
async def on_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if doc.file_size and doc.file_size > 8_000_000:
        await update.message.reply_text("Trop lourd, 8 Mo maximum.")
        return
    await context.bot.send_chat_action(update.effective_chat.id, ChatAction.TYPING)
    f = await doc.get_file()
    txt = texte_document(doc.file_name or "", bytes(await f.download_as_bytearray()))
    if txt is None:
        await update.message.reply_text(
            "Format non lu. J'accepte txt, md, csv, json et pdf.")
        return
    txt = re.sub(r"\n{3,}", "\n\n", txt).strip()
    if not txt:
        await update.message.reply_text("Ce document ne contient pas de texte.")
        return
    question = (update.message.caption or "").strip() or \
        "Résume ce document en quelques lignes."
    coupe = " …(tronqué)" if len(txt) > MAX_DOC else ""
    await think(f"[document {doc.file_name}]\n{txt[:MAX_DOC]}{coupe}\n\n{question}",
                update, context, memo=f"[document {doc.file_name}] {question}")


AIDE = """Joorvis. Écris, parle, envoie une photo ou un document.
...
Le reste part au modèle, qui enchaîne ses outils tout seul :
web et recherche approfondie, météo, Wikipédia, calcul exact,
devises, crypto, notes, tâches, mémoire longue durée.

Une photo : je la lis, je transcris le texte qu'elle contient.
Un document txt, md, csv, json ou pdf : je le résume, ou je
réponds à la question mise en légende.

/r rappels   /t tâches   /notes [mot]   /memoire   /brief
/export   /purge   /undo   /stats   /reset
...
async def cmd_undo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if quoi == "rappel":
        drop_reminder(context.job_queue, cid, rid)
    elif quoi == "note":
        drop_note(cid, rid)
    else:
        with db() as con:
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    with db() as con:
        t = con.execute("SELECT COALESCE(SUM(llm),0) l, COALESCE(SUM(local),0) o, "
                        ...
                        "FROM stats").fetchone()
        sem = con.execute("SELECT * FROM stats ORDER BY jour DESC "
                          "LIMIT 7").fetchall()
    l, o = t["l"], t["o"]
    total = l + o
    out = [
        ...
        f"Tokens    {nombre(t['i'] + t['s'])} depuis le début",
    ]
    if sem:
        out.append("")
        for r in sem:
            d = datetime.fromisoformat(r["jour"])
            tok = (r["tok_in"] or 0) + (r["tok_out"] or 0)
            out.append(f"{ABBR[d.weekday()]} {d.day:>2}   "
                       f"{r['local'] or 0:>3} local   {r['llm'] or 0:>3} modèle   "
                       f"{nombre(tok)} tok")
    await update.message.reply_text("\n".join(out))
...
async def cmd_reset(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.chat_data.pop("hist", None)
    await update.message.reply_text("Fil oublié. Ma mémoire longue est intacte.")


@private
async def cmd_notes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    motif = " ".join(context.args) if context.args else ""
    ns = find_notes(update.effective_chat.id, motif, 8)
    if not ns:
        await update.message.reply_text(
            f"Aucune note pour « {motif} »." if motif else "Aucune note.")
        return
    await update.message.reply_text("\n\n".join(
        f"{n['cree'][:10]} · {n['texte'][:300]}" for n in ns))


@private
async def cmd_export(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Toutes tes données dans un fichier, lisible sans le bot."""
    cid = update.effective_chat.id
    data = {
        "export": now().isoformat(timespec="seconds"),
        "rappels": [{"texte": r["texte"], "quand": r["_label"]}
                    for r in fetch_reminders(cid)],
        "taches": [t["texte"] for t in fetch_tasks(cid)],
        "notes": [{"date": n["cree"][:10], "texte": n["texte"]}
                  for n in find_notes(cid, "", 1000)],
        "memoire": {f["cle"]: f["valeur"] for f in load_facts(cid)},
    }
    buf = BytesIO(json.dumps(data, ensure_ascii=False, indent=2).encode())
    await update.message.reply_document(
        buf, filename=f"joorvis-{now():%Y%m%d}.json",
        caption=f"{len(data['rappels'])} rappels · "
                f"{len(data['taches'])} tâches · {len(data['notes'])} notes")


PURGES = {"rappels": "reminders", "taches": "tasks",
          "notes": "notes", "memoire": "facts"}


@private
async def cmd_purge(update: Update, context: ContextTypes.DEFAULT_TYPE):
    kb = InlineKeyboardMarkup(
        [[InlineKeyboardButton(f"Effacer {k}", callback_data=f"pg:{k}")]
         for k in PURGES] +
        [[InlineKeyboardButton("Tout effacer", callback_data="pg:tout")]])
    await update.message.reply_text("Qu'est-ce que j'efface ?", reply_markup=kb)
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if kind == "pg":
        ...
        await q.edit_message_text(
            f"Effacer {rest} ? C'est définitif.",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("Oui, efface", callback_data=f"pgok:{rest}"),
                InlineKeyboardButton("Non", callback_data="pgno:")]]))
        return

    if kind == "pgno":
        await q.answer("Rien touché")
        await q.edit_message_text("Rien effacé.")
        return

    if kind == "pgok":
        tables = list(PURGES.values()) if rest == "tout" else [PURGES.get(rest)]
        n = 0
        with db() as con:
            for t in filter(None, tables):
                n += con.execute(f"DELETE FROM {t} WHERE chat_id = ?",
                                 (chat_id,)).rowcount
        if rest in ("rappels", "tout"):
            for job in context.job_queue.jobs():
                if job.name != "brief":
                    job.schedule_removal()
        await q.answer("Effacé")
        await q.edit_message_text(f"🗑 {n} élément{'s' if n > 1 else ''} effacé"
                                  f"{'s' if n > 1 else ''}.")
        return
...
async def brief_redige(chat_id: int) -> str:
    """Le brief brut, repassé par le modèle. Si le modèle est muet, on garde
    la version assemblée : le brief part tous les matins, quoi qu'il arrive."""
    brut = await texte_brief(chat_id)
    if brut.count("\n") == 0:
        return brut
    try:
        r = await appel_modele([
            system_msg(chat_id),
            {"role": "user", "content":
                "Réécris ce brief du matin en 4 lignes maximum, ton direct, "
                "texte brut, garde les heures exactes et n'invente rien. "
                "Commence par la date.\n\n" + brut}], outils=False)
        texte = (r.choices[0].message.content or "").strip()
        if r.usage:
            stat("llm", r.usage.prompt_tokens, r.usage.completion_tokens)
        return texte or brut
    except Exception as e:
        log.warning("brief rédigé indisponible : %s", e)
        return brut
...
async def job_brief(context: ContextTypes.DEFAULT_TYPE):
    try:
        await context.bot.send_message(context.job.chat_id,
                                       await brief_redige(context.job.chat_id))
    except TelegramError as e:
        log.error("brief : %s", e)
...
async def cmd_brief(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(await brief_redige(update.effective_chat.id))
...
async def on_startup(app):
    ...
    await app.bot.set_my_commands([
        ...
        BotCommand("t", "Tâches"),
        BotCommand("notes", "Mes notes"),
        BotCommand("memoire", "Ce que je retiens de toi"),
        BotCommand("brief", "Le point du jour"),
        BotCommand("export", "Télécharger mes données"),
        BotCommand("purge", "Tout effacer"),
